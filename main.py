...
LICENSE_FILE = "clients.json"
SAVED_LICENSE = "license_config.json"
...
def validate_license_key():
    ...
    if not os.path.exists(LICENSE_FILE):
        print_error("⚠️ License file 'clients.json' is missing.")
        sys.exit()
    ...
    try:
        ...
        print_error("⚠️ Failed to read or parse 'clients.json'.")
        sys.exit()

    # Check for saved key
    saved_key = None
    if os.path.exists(SAVED_LICENSE):
        try:
            with open(SAVED_LICENSE, "r") as f:
    ...
    while True:
        if saved_key:
            ...
            print_info(f"🔑 Using saved key: {license_key}")
        else:
            print_input_prompt("🔑 Enter your license key")
        ...
        if not lic.get("active", False):
            print_error("⛔ This key is deactivated.")
            if saved_key:
                os.remove(SAVED_LICENSE)
                saved_key = None
            continue
        ...
        try:
            ...
            if datetime.datetime.now() > expires:
                print_error("📅 This key has expired.")
                if saved_key:
                    os.remove(SAVED_LICENSE)
                    saved_key = None
                continue
        except:
            print_error("⚠️ Invalid expiration date.")
            if saved_key:
                os.remove(SAVED_LICENSE)
                saved_key = None
            continue
        ...
        if device_id not in users:
            if len(users) >= max_users:
                print_error("🚫 Maximum devices reached.")
                if saved_key:
                    os.remove(SAVED_LICENSE)
                    saved_key = None
                continue
            ...
            print_success("✅ License verified.")

        # Save key locally if not already saved
        if not saved_key:
            with open(SAVED_LICENSE, "w") as f:
